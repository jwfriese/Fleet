...
for storyboardFolder in "$TEST_HOST_INTERMEDIATE_BUILD_FOLDER"/*.storyboardc; do
    STORYBOARD_NAME=$(echo "$storyboardFolder" | rev | cut -d '/' -f1 | rev | cut -d '.' -f1)
    mkdir "$COPY_DESTINATION_PATH"/"$STORYBOARD_NAME"
    cp "$TEST_HOST_INTERMEDIATE_BUILD_FOLDER"/"$STORYBOARD_NAME".storyboardc/Info.plist "$COPY_DESTINATION_PATH"/"$STORYBOARD_NAME"
done
