...
for storyboardFolderTemplate in $(find "$TEST_HOST_INTERMEDIATE_BUILD_FOLDER" -name "*.storyboardc" | sed 's/ /\/\//g'); do
storyboardFolder=$(echo "$storyboardFolderTemplate" | sed 's/\/\// /g')
